...
import { createQueries, type AiDexDatabase } from '../db/index.js';
...
export function summary(params: SummaryParams): SummaryResult {
    ...
    return withProjectDb(
        projectPath, true,
        (error) => ({ success: false, name: '', content: '', autoGenerated: { entryPoints: [], mainTypes: [], fileCount: 0, languages: [] }, error }),
        (db, queries) => {
            try {
...
export function tree(params: TreeParams): TreeResult {
    ...
    return withProjectDb(
        projectPath, true,
        (error) => ({ success: false, root: '', entries: [], totalFiles: 0, error }),
        (db, queries) => {
            try {
                const files = queries.getAllFiles();
